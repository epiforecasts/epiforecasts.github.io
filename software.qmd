...
## The weekly job records to the stats-data branch rather than to main, so
## that it needs no exception to the ruleset protecting default branches.
## Read from there, falling back to a local file so the page still renders
## before the branch exists or if the fetch fails.
stats_url <- paste0(
...
if (!is.null(stats) && nrow(stats) > 0) {
  # Take each repository's most recent figure rather than the newest date's:
  # one week where cranlogs is unreachable writes NAs for everything, and
  # keying on the newest date alone would drop every figure from the page
  # even though last week's are sitting in the same file. Bounded so a
  # long outage shows nothing rather than something months out of date.
...
all_packages %>%
  purrr::map_chr(function(e) {
    ...
    # is distributed elsewhere, not unused.
    downloads_text <- if (!is.null(downloads)) {
      ...
      # Only link where the registry is known. A package from the registry is
      # served by the epiforecasts universe whatever repo it lives in, so
      # epimixr at sbfnk/epimixr still resolves there. For an extras entry we
      # cannot know: its owner is not necessarily a universe, and guessing
      # produced dead links. Those carry a "Part of" link to their own project
      # instead, so say nothing rather than something wrong.
